...
static uint8_t ST7735_WriteChar(uint16_t x, uint16_t y, char ch, tFont* font, uint16_t color, uint16_t bgcolor) {
    uint32_t i, b, j;
    uint8_t chr;
    uint8_t n, m;
    uint8_t sym_width;
    ...
    if(!chr) return 0;
    sym_width = font->chars[chr].image->width;
...
uint16_t ucg_WriteString(ucg_t* ucg, uint16_t x, uint16_t y, const char* str) {
    ST7735_Select();
    uint8_t chr;
    if (ucg->font) {
        while(*str) {
